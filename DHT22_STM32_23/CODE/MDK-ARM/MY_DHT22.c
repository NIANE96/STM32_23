//Header files
#include "MY_DHT22.h"

//Bit fields manipulations
#define bitRead(value, bit) (((value) >> (bit)) & 0x01)
#define bitSet(value, bit) ((value) |= (1UL << (bit)))
#define bitClear(value, bit) ((value) &= ~(1UL << (bit)))
#define bitWrite(value, bit, bitvalue) (bitvalue ? bitSet(value, bit) : bitClear(value, bit))

//1. One wire data line
static GPIO_TypeDef* oneWire_PORT;
static uint16_t oneWire_PIN;
static uint8_t oneWirePin_Idx;

//*** Functions prototypes ***//
//OneWire Initialise
void DHT22_Init(GPIO_TypeDef* DataPort, uint16_t DataPin)
{
	oneWire_PORT = DataPort;
	oneWire_PIN = DataPin;
	for(uint8_t i=0; i<16; i++)
	{
		if(DataPin & (1 << i))
		{
			oneWirePin_Idx = i;
			break;
		}
	}
}
//Change pin mode
static void ONE_WIRE_PinMode(OnePinMode_Typedef mode)
{
	GPIO_InitTypeDef GPIO_InitStruct;
	GPIO_InitStruct.Pin = GPIO_PIN_9; //PIN A9 equivalent D8 sur shield
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	
	if(mode == ONE_OUTPUT)
	{
//		oneWire_PORT->MODER &= ~(3UL << 2*oneWirePin_Idx);  //Reset State
//		oneWire_PORT->MODER |= (0x01 << 2*oneWirePin_Idx); //Output Mode
		GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	}
	else if(mode == ONE_INPUT)
	{
//		oneWire_PORT->MODER &= ~(3UL << 2*oneWirePin_Idx);  //Input Mode
		GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
	}
	
	HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
}	
//One Wire pin HIGH/LOW Write
static void ONE_WIRE_Pin_Write(bool state)
{
	if(state) HAL_GPIO_WritePin(oneWire_PORT, oneWire_PIN, GPIO_PIN_SET);
	else HAL_GPIO_WritePin(oneWire_PORT, oneWire_PIN, GPIO_PIN_RESET);
}
static bool ONE_WIRE_Pin_Read(void)
{
	return (1&HAL_GPIO_ReadPin(oneWire_PORT, oneWire_PIN));
}

//Définition du temps en microsecondes
static void DelayMicroSeconds(uint32_t uSec)
{
	uint32_t uSecVar = uSec;
	uSecVar = uSecVar* ((SystemCoreClock/1000000)/3);
	while(uSecVar--);
}

//Lancer l'acquisition des données
static void DHT22_StartAcquisition(void)
{
	//Change data pin mode to OUTPUT
	ONE_WIRE_PinMode(ONE_OUTPUT);
	
	//Put pin LOW
	ONE_WIRE_Pin_Write(0);
	
	//Délai de 800 us
	//eq T be : Host the stard signal down time
	DelayMicroSeconds(800);
	
	//Bring pin HIGH
	ONE_WIRE_Pin_Write(1);
	
	//Délai de 30 us
	//T go : Bus master has released time
	DelayMicroSeconds(30);
	//Set pin as input
	ONE_WIRE_PinMode(ONE_INPUT);
}
//Lecture des 4*8 données du DHT22
static void DHT22_ReadRaw(uint8_t *data)
{
	uint32_t rawBits = 0UL;
	uint8_t checksumBits=0;
	
	DelayMicroSeconds(40); //Délai de 40 us
	while(!ONE_WIRE_Pin_Read());
	while(ONE_WIRE_Pin_Read());
	
	for(int8_t i=31; i>=0; i--)
	{
		while(!ONE_WIRE_Pin_Read());
		DelayMicroSeconds(40); //Délai de 40 us
		
		if(ONE_WIRE_Pin_Read())
		{
			rawBits |= (1UL << i);
		}
		while(ONE_WIRE_Pin_Read());
	}
	
	for(int8_t i=7; i>=0; i--)
	{
		while(!ONE_WIRE_Pin_Read());
		DelayMicroSeconds(40); //Délai de 40 us
		
		if(ONE_WIRE_Pin_Read())
		{
			checksumBits |= (1UL << i);
		}
		while(ONE_WIRE_Pin_Read());
	}
	
	
	//Copie des données dans des tableaux avec masquage pour une position précise
	data[0] = (rawBits>>24)&0xFF; //Humudity High
	data[1] = (rawBits>>16)&0xFF; //Humidity Low
	data[2] = (rawBits>>8)&0xFF;  //Temperature High
	data[3] = (rawBits>>0)&0xFF;  //Temperature Low
	data[4] = (checksumBits)&0xFF; //Parity Bit
}

//Récupération des données de température et d'humidité
bool DHT22_GetTemp_Humidity(float *Temp, float *Humidity)
{
	uint8_t dataArray[6], myChecksum;
	uint16_t Temp16, Humid16;
	
	//Lancementacquisition
	DHT22_StartAcquisition();
	
	//Acquisition des données
	DHT22_ReadRaw(dataArray);
	
	//Addition des 4*8 données pour comparaison avec le Parity bit
	myChecksum = 0;
	
	for(uint8_t k=0; k<4; k++) //Récupération des données et addition des bits
	{
		myChecksum += dataArray[k];
	}
	if(myChecksum == dataArray[4]) //Si égalité entre l'addition des 4*8 données et le Parity bit
	{
		Temp16 = (dataArray[2] <<8) | dataArray[3]; // Assemblage données Temperature
		Humid16 = (dataArray[0] <<8) | dataArray[1]; // Assemblage données Humidité
		
		*Temp = Temp16/10.0f; //Calcul Temperature
		*Humidity = Humid16/10.0f; //Calcul Humidité
		
		return 1;
	}
	return 0;
}

